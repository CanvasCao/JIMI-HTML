...
(function (w, d, $, undefined) {
    function LightUpMask(container, data) {
        ...
        this.data.showCallback = $('.light').animate({opacity: 0}, 'fast', 'swing');
        this.data.hideCallback = $('.light').animate({opacity: 1}, 'fast', 'swing');//maskHide所以light要show
        this.config = {
            imgW: 60
        };
    ...
    LightUpMask.prototype = {
        ...
        show: function (data) {
            ...
            if (data.length > 0) {
                for (i = 0; i < data.length; i++) {
                    $(that.C).find('.maskSectionCon').append(
                        ...
                        "<div class='maskSectionImg'> <img src='" + data[i].userImgUrl + "' width='" + 40 + "' /> </div>" +
            ...
            setTimeout(function () {
                that.freshInputBox();
                $(that.C).fadeIn('normal', 'swing', that.data.showCallback);
        ...
        hide: function () {
            var that = this;
            this.ifShow = false;
            $(that.C).fadeOut('normal', 'swing', that.data.hideCallback);
            $(that.C).find('.lightUpMaskBottom').animate({opacity: 0, bottom: -300}, 'normal', 'swing');
        },
